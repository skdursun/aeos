...
10. TASK-0273: Implement CLI task plan command planner integration. Purpose: wire the existing command boundary to parser, mapper, planner wiring, dependency-injected planner, human rendering, JSON rendering, and process exit-code handling without execution or writes. Likely files: `apps/cli/src/commands.ts`, `apps/cli/scripts/smoke.mjs`. Verification: `pnpm --filter @aeos/cli smoke`. Effort: High. Classification: Code.
11. TASK-0274: Fix AEOS global audit integration findings for task plan. Purpose: complete authoritative verifier proof, validation/parser consistency, mapper authoritative safety proof, and directly related fixture/docs/context alignment. Likely files: `packages/core/src/task-validation.ts`, `packages/core/src/task-plan-input-parser.ts`, `packages/core/src/task-contract-mapper.ts`, `packages/core/src/task-plan-file-planner-wiring-logic.ts`, `packages/core/scripts/smoke.mjs`, `apps/cli/scripts/smoke.mjs`, `apps/cli/fixtures/tasks/valid-task.json`, listed docs. Verification: `pnpm --filter @aeos/core verify`, `pnpm --filter @aeos/cli smoke`, `pnpm cli:verify`, `pnpm check`. Effort: High. Classification: Code.
12. TASK-0275: Post-audit regression review. Purpose: review the hardened parser, mapper, and planner integration after TASK-0274 without adding execution, adapters, audit runtime, verifier runtime, persistence, or resume runtime. Likely files: audit notes and focused docs/tests if findings require them. Verification: `pnpm check`, `pnpm cli:verify`, targeted smoke suites. Effort: High. Classification: Review.
13. TASK-0276: Update task plan planner integration usage docs. Purpose: document any remaining post-review behavior drift, supported fallback limits, JSON-only output, fail-closed statuses, and remaining non-goals. Likely files: listed task plan usage docs. Verification: `git status --short`. Effort: Medium. Classification: Docs.
